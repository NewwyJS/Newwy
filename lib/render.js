...
export function renderStyles(route, components) {
  ...
  const parsedStyle = parseStyle(style).parsed;
  const parsedVars = parseStyle(style).rootVars;

  return `<style>${parsedVars} ${parsedStyle}</style>`;
}
...
export function renderBody(route, deps, data) {
  ...
  const target = outputdir + '/autoload.js';

  const script = `import createApp from '/node_modules/nuejs-core/src/browser/nue.js';
  ...
  const html = `
  ${styles}
  <script type="module" src="/.newwy/autoload.js?v=${moment().unix()}"></script>
  <script type="module" src="/node_modules/newwy/lib/browser/preloader.js">
