...
export async function buildRoutes() {
  ...
  for (const file of files) {
    ...
    const bareStyle = fileContents.match(/<newwy:style>(.*?)<\/newwy:style>/s);
    const style = bareStyle
      ? bareStyle[0].replace(/<newwy:style>(.*?)<\/newwy:style>/s, "$1")
      : "";

    const body = fileContents
      .replace(/<newwy:head>(.*?)<\/newwy:head>/s, "")
      .replace(/<newwy:style>(.*?)<\/newwy:style>/s);
...
export async function buildComponents() {
  ...
  for (const file of files) {
    ...
    const outputFilePath = outputdir + "/" + outputFile;
    const fileContents = fs.readFileSync(file, "utf-8");
    const body = fileContents.replace(/<newwy:style>(.*?)<\/newwy:style>/s);
    const compiled = compile(body);
